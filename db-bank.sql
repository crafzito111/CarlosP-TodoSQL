...
--llamando clients cuentas y balances 
create view v_clients_account as
select 
c.id,
c.first_name,
c.last_name,
...
select * from v_clients_balance;

--llamado cuentas y tipos de cuentas 

create view v_account_types as
select
a.account_no, 
a.client_id,
at1."name" as tipo_de_cuenta
from accounts as a 
left join account_types as at1 on a.type = at1.id;

select * from v_account_types;

--llama las transacciones, su monto y sus tipos
...
left join transactions_types as tt on t.transaction_type = tt.id 

select * from v_transactions_detail;

--llama las transacciones, su monto y todos sus tipos
...
tt."name" as tipo_de_transaccion
from transactions as t 
right join transactions_types as tt on t.transaction_type = tt.id 
